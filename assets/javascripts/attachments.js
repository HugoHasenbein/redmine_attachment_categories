...
function addFile(inputEl, file, eagerUpload) {
  ...
  if ($('#attachments_fields').children().length < 10) {
    ...
    fileSpan.append(
        ...
        $('<input>', { type: 'text', 'class': 'description', name: 'attachments[' + attachmentId + '][description]', id: 'attachments_' + attachmentId + '_description', maxlength: 255, placeholder: $(inputEl).data('description-placeholder') } ).toggle(!eagerUpload),
        $('<a>&nbsp</a>').attr({ href: "#", 'class': 'remove-upload' }).click(removeFile).toggle(!eagerUpload)
    ).appendTo('#attachments_fields');
    
    observeAutocompleteField('attachments_' + attachmentId + '_description', '/auto_completes/attachment');
